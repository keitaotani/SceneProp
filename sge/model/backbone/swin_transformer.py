...
class WindowAttention(torch.jit.ScriptModule):
...
class SwinTransformerBlock(torch.jit.ScriptModule):
    ...
    def __init__(self, dim, num_heads, window_size:int=7, shift_size=0,
    ...
    @torch.jit.script_method
    def forward(self, x, mask_matrix, H:int, W:int):
        """ Forward function.
        Args:
...
class PatchMerging(torch.jit.ScriptModule):
    """ Patch Merging Layer
    Args:
        dim (int): Number of input channels.
...
class BasicLayer(torch.jit.ScriptModule):
...
class PatchEmbed(torch.jit.ScriptModule):
