...
class BoxList:
    ...
    def __init__(self,
        bbox: Union[torch.Tensor, np.ndarray, list[list]],
        ...
        mode : str
            It represents the order of `bbox`. It should be ``'xyxy'`` or ``'xywh'``.
        """
    ...
    def add_field(self, field: str, field_data):
    ...
    def convert(self, mode: str):
        """ Convert the box format from ``'xyxy'`` to ``'xywh'`` or ``'xywh'`` to ``'xyxy'``.
        It returns converted :class:`~BoxList`, note that it is not an in-place method.
    ...
    def resize(self, size: tuple[int, int], *args, **kwargs):
        ...
        for k, v in self.extra_fields.items():
            if type(v) != torch.Tensor and hasattr(v, "resize"):  # torch.Tensor has resize but it's not what we want
                v = v.resize(size, *args, **kwargs)
            bbox.add_field(k, v)
    ...
    def clip_to_image(self, remove_empty: bool = True):
        TO_REMOVE = 1
    ...
    def center(self):
        """ Returns the center of each box. """
        if self.mode == 'xyxy':
            box = self.bbox
            center = torch.stack([(box[:, 0] + box[:, 2]) / 2, (box[:, 1] + box[:, 3]) / 2], dim=-1)
        elif self.mode == 'xywh':
            box = self.bbox
            center = torch.stack([box[:, 2], box[:, 3]], dim=-1)
        else:
            raise RuntimeError("Should not be here")
            
        return center

    def copy_with_fields(self, fields: Union[list, tuple, str]):
        """ Copy the bounding boxes and specified fields.

        Parameters
        ----------
        fields : Union[list, tuple, str]
            The field name or the list of field names which should be copied.
        """
    ...
    @staticmethod
    def concate_box_list(list_of_boxes: Union[list[BoxList], tuple[BoxList]]) -> BoxList:
